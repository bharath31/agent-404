...
describe("ClaudeBot Probe & Standing Audit (BAT-38, BAT-39)", () => {
	...
	describe("deep audit mode (BAT-22)", () => {
		...
		it("does not claim a clean site when discovery finds no pages", async () => {
			vi.mocked(discoverDemoPages).mockResolvedValue({
				domain: "empty.example.com",
				pages: [],
				source: "none",
				error: "No discoverable pages",
			});
			const { app } = createTestApp();
			...
			const createRes = await app.request("/api/audit", {
				...
				body: JSON.stringify({ domain: "empty.example.com", deadPath: "/gone", deep: true }),
			});
