...
<head>
  ...
  <title>agent-404 — Agent-friendly 404 recovery for AI crawlers &amp; LLMs</title>
  <meta name="description" content="Serve agent-friendly 404 recovery at the HTTP layer. Ranked suggestions in HTML, schema.org JSON-LD, and Link headers — so ClaudeBot, GPTBot, and PerplexityBot recover without JavaScript.">
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-btn:hover { border-color: var(--text-secondary); }
    ...
    /* Live Animated URL Matcher Demo */
    .demo-window {
      max-width: 620px;
    ...
    /* Comparison Terminal: Before vs After */
    .comparison-grid {
      display: grid;
    ...
    .terminal-header.bad { color: var(--red); }
    .terminal-header.good { color: var(--green); }
    .terminal-body {
      padding: 1rem;
      font-family: var(--mono);
      font-size: 0.76rem;
    ...
    .terminal-body .term-cmd { color: var(--text); }
    .terminal-body .term-status-bad { color: var(--red); font-weight: 600; }
    .terminal-body .term-status-good { color: var(--green); font-weight: 600; }
    .terminal-body .term-header { color: #60a5fa; }
    .terminal-body .term-json { color: #facc15; }
    .terminal-body .term-comment { color: #52525b; }
...
<body>
  <div class="container">
    <nav>
      <div class="logo">
        agent<span>-</span>404
        <span class="logo-badge">HTTP-Layer</span>
      </div>
      <div class="links">
    ...
    <div class="hero">
      <div class="badge">
        ...
        <span>Open Source &middot; MIT Licensed &middot; pgvector Ranked</span>
      </div>
      <h1>When AI crawlers hit dead links,<br><span class="highlight">give them the right page</span></h1>
      <p class="lead">GPTBot, ClaudeBot, and PerplexityBot do not execute JavaScript. agent-404 intercepts 404s at the HTTP layer to inject ranked suggestions, schema.org JSON-LD, and Link alternate headers — so AI systems recover automatically.</p>
      
      <div class="hero-actions">
        <a href="/demo" class="btn btn-primary">Audit Your Site (Free) &rarr;</a>
    ...
    <div class="proof-strip">
      <div class="proof-stat">
        <span class="val highlight">2.38%</span>
        <span class="label">of ChatGPT citations land on 404s (3&times; higher than Google)</span>
      </div>
      <div class="proof-stat">
        <span class="val">0%</span>
        <span class="label">of AI crawlers execute JavaScript (Vercel: 500M+ GPTBot fetches)</span>
      </div>
      <div class="proof-stat">
        <span class="val highlight">&lt;25ms</span>
        <span class="label">cached pgvector response time on dead link interception</span>
      </div>
    </div>
    ...
    <div class="demo-window">
      <div class="demo-header">
        ...
        <div class="demo-title">agent-404 &mdash; live vector resolution</div>
      </div>
    ...
    <div class="section" style="padding-top: 1rem;">
      <h2>Why HTTP-layer recovery is mandatory</h2>
      <p class="sub">AI crawlers parse raw HTTP responses. Client-side script tags never run in crawler environments.</p>

      <div class="comparison-grid">
        <div class="terminal-card">
          <div class="terminal-header bad">
            <span>Standard 404 Response</span>
            <span>Agent gives up / Hallucinates</span>
          </div>
          <div class="terminal-body"><span class="term-cmd">$ curl -sI -A "ClaudeBot" https://yoursite.com/docs/v2/auth</span>

<span class="term-status-bad">HTTP/1.1 404 Not Found</span>
Content-Type: text/html

<span class="term-comment">&lt;!-- Client script tag is never executed by crawler --&gt;</span>
&lt;h1&gt;Page Not Found&lt;/h1&gt;
&lt;p&gt;Sorry, this page does not exist.&lt;/p&gt;</div>
        </div>

        <div class="terminal-card">
          <div class="terminal-header good">
            <span>With agent-404 Middleware</span>
            <span>Agent recovers automatically</span>
          </div>
          <div class="terminal-body"><span class="term-cmd">$ curl -sI -A "ClaudeBot" https://yoursite.com/docs/v2/auth</span>

<span class="term-status-good">HTTP/1.1 404 Not Found</span>
<span class="term-header">Link: &lt;https://yoursite.com/docs/v3/auth&gt;; rel="alternate"</span>
Content-Type: text/html

<span class="term-json">&lt;script type="application/ld+json"&gt;
{
  "@context": "https://schema.org",
  "@type": "ItemList",
  "itemListElement": [{
    "url": "https://yoursite.com/docs/v3/auth",
...
<span class="kw">import</span> { agent404Worker } <span class="kw">from</span> <span class="str">"agent-404/adapters/cloudflare"</span>;
  ...
  fetch: <span class="fn">agent404Worker</span>(handler, {
    apiKey: env.<span class="fn">AGENT404_PUBLIC_KEY</span>,
      ...
      <div class="code-preview" id="snippet-express" style="display:none">
        <pre><span class="comment">// app.ts — Express middleware</span>
<span class="kw">import</span> { recoverExpress404 } <span class="kw">from</span> <span class="str">"agent-404/adapters/express"</span>;
...
app.<span class="fn">use</span>(<span class="fn">recoverExpress404</span>({ apiKey: process.env.<span class="fn">AGENT404_PUBLIC_KEY</span>! }));</pre>
      ...
      <div class="code-preview" id="snippet-script" style="display:none">
        <pre><span class="comment">&lt;!-- Zero-config fallback for human browsers &amp; JS-capable agents --&gt;</span>
...
&gt;&lt;/<span class="kw">script</span>&gt;</pre>
      ...
      <div class="domain-cta">
        <h4>Register your domain to get API keys</h4>
        <p>Enter your domain. We'll email a one-time login code — no password required.</p>
    ...
    <div class="audit-banner">
      <div>
        <h3>Test your site before installing</h3>
        <p>Run a free audit on your live sitemap. See what your 404 returns to ClaudeBot today and test real-time vector matching on broken URLs.</p>
      </div>
    ...
    <div class="section">
      <h2>How it works</h2>
      <p class="sub">Three steps to recover AI crawler traffic before agents give up or hallucinate.</p>
      ...
      <div class="flow">
        ...
        <div class="flow-card">
          ...
          <div>
            ...
            <p>Returns ranked HTML links for human visitors, schema.org ItemList JSON-LD and Link alternate headers for crawlers, and structured JSON on Accept: application/json.</p>
    ...
    <div class="section">
      ...
      <div class="use-cases">
        <div class="use-case">
          <div class="icon-row">
            <div class="icon">🤖</div>
            <h3>Coding Assistants (Claude, Cursor, Copilot)</h3>
          </div>
          <p>LLMs follow documentation URLs baked into model weights. When your docs restructure, agents hit 404s and hallucinate. agent-404 serves structured JSON-LD so models self-correct automatically.</p>
        </div>
        ...
        <div class="use-case">
          ...
          <p>Retrieval systems index URLs that break after API version bumps. Instead of returning empty context or broken citations, agents receive the matching destination endpoint immediately.</p>
        </div>
        ...
        <div class="use-case">
          <div class="icon-row">
            <div class="icon">🔌</div>
            <h3>MCP Server &amp; Tool Resource Endpoints</h3>
          </div>
          <p>Model Context Protocol tool calls requesting docs or assets receive ranked alternative endpoints rather than an unparseable generic error page.</p>
        </div>
      </div>
    </div>

    <div class="cta">
      <h2>Stop losing AI traffic to dead links</h2>
