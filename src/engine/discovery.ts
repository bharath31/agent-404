...
export type DemoPage = { url: string; title: string; description?: string };
 ...
 */
export function detectBlockedResponse(status: number, body: string | null): string | null {
 ...
 */
export async function fetchDemoResponse(url: string): Promise<FetchMeta> {
 ...
 */
export function hasRelevantPages(pages: DemoPage[], deadPath: string): boolean {
 ...
 */
export async function discoverDemoPages(
	domain: string,
	deadPath: string,
 ...
 */
export async function followChildLlmsTxt(
 ...
 */
export async function findSitemapUrls(domain: string, deadPath: string): Promise<string[]> {
	const found: string[] = [];
	const seen = new Set<string>();
 ...
 */
export async function fetchDemoText(url: string): Promise<string | null> {
	const controller = new AbortController();
	const timeout = setTimeout(() => controller.abort(), DEMO_FETCH_TIMEOUT_MS);
	try {
		const resp = await fetch(url, {
 ...
 */
export async function fetchDemoSitemapXml(url: string): Promise<string | null> {
	const controller = new AbortController();
	const timeout = setTimeout(() => controller.abort(), DEMO_FETCH_TIMEOUT_MS);
	try {
		const resp = await fetch(url, {
 ...
 */
export function scoreChildSitemap(childUrl: string, deadPath: string): number {
 ...
 */
export function parsePlainTextSitemap(text: string, filterDomain?: string): DemoPage[] {
	const pages: DemoPage[] = [];
	const seen = new Set<string>();
 ...
 */
export async function fetchDemoSitemap(
	url: string,
	deadPath: string,
	depth: number,
	filterDomain?: string,
 ...
 */
export async function crawlDemoLinks(
	domain: string,
	deadPath: string,
): Promise<CrawlResult> {
 ...
 */
export function prioritizeLocs(locs: string[], deadPath: string): string[] {
...
export function extractDemoLocs(xml: string, parentTag: string): string[] {
	const urls: string[] = [];
...
export function demoTitleFromUrl(url: string): string {
	try {
