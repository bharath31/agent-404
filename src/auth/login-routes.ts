...
loginRoutes.post(
	...
	async (c) => {
		...
		if (!cfg || !emailCfg) return authNotConfigured(c);
		...
		try {
			await store.saveOtp(email, hashOtp(code), expiresAt);
			await sendOtpEmail(emailCfg, email, code);
		} catch (error) {
			// Don't leave a pending code we couldn't deliver.
