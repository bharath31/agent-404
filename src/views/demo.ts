  ...
  <script>
    ...
    async function runDeepAudit() {
      ...
      try {
		...
		if (res.ok) {
			const report = await res.json();
			showAuditReport(report, 'https://' + lastLiveAudit.domain + lastLiveAudit.deadPath);
			showToast(report.analysis ? 'Full site audit complete' : 'Could not crawl this site — showing probe results');
		} else {
